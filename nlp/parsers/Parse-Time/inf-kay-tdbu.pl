...
test:- tell('rez_ded_kay_td_bu.doc'),parse(X).
testf:-tell('rez_ded_kay_td_bu-trasare.doc'),assert(trasare),parse(X),nl,nl,listing(word),listing(stored),listing(key_index),retract(trasare).
