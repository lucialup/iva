...
test_run(E,P,A):-exe(E),ut(A,E,[]),ut(A,P,[]).

test_sentence(E):-(ut(Tree,E,[])->write('Parse tree:'),nl,write(Tree),nl;write('No parse found for: '),write(E),nl,fail).
...
test_all:-tell('td-time.doc'),
    findall((E,A),(exe(E),once(test_run(E,_,A))),L),write_l(L).

write_l([]):-!.
write_l([(E,A)|T]):-write('Propozitie: '),write(E),nl,write('Arb: '),write(A),nl,nl,write_l(T).
